...
public class Line: Shape {
	...
	public convenience init(canvas: Canvas) {
		...
		self.init(canvas: canvas, start: start, end: end)
    }
    
    /// Creates a Line with the given start and end point. Note that `Point(x: 0, y: 0)` is the center of the canvas.
    ///
    /// - Parameter start: The point at which to start drawing the line.
    /// - Parameter end: The point at which to stop drawing the line.
    /// - Parameter thickness: The thickness of the line. The default value is 0.5.
    /// - localizationKey: Line(start:end:thickness:)
	public init(canvas: Canvas, start: Point, end: Point, thickness: Double = 0.5) {
        self.start = start
        self.end = end
...
extension CanvasViewController {
	public func Line() -> Line {
		return Shapes.Line(canvas: canvas)
	}
	...
	/// - Parameter thickness: The thickness of the line. The default value is 0.5.
	/// - localizationKey: Line(start:end:thickness:)
	public func Line(start: Point, end: Point, thickness: Double = 0.5) -> Line {
		return Shapes.Line(canvas: canvas, start: start, end: end, thickness: thickness)
	}
}
